...
# Ported from xomify-infrastructure's lambda_authorizer.tf. The Lambda's
# own code (lambdas/authorizer/handler.py in xomforms-backend) validates
# real Cognito-issued JWTs against the shared pool's JWKS (see
# data_cognito.tf) rather than xomify's homegrown HS256 API_SECRET_KEY --
# still email-keyed context out, same Allow/Deny policy shape.
